...
def scale_data(train_data, val_data, test_data):
    ...
    return train_data.reshape(-1, 224, 224), val_data.reshape(-1, 224, 224), test_data.reshape(-1, 224, 224)
...
def compile_and_train_model(model, train_dataset, val_dataset, BATCH_SIZE, EPOCHS, class_weights, checkpoint):
    # Compile the model
...
def main(DATA_PATH, MODELS, GPUS):
    ...
    model = load_model('/home/u956278/research_workshop/github_models/best_model_autokeras', custom_objects={'tfa.metrics.F1Score':tfa.metrics.F1Score,'ak.CUSTOM_OBJECTS':ak.CUSTOM_OBJECTS})
    model = model.save('/home/u956278/research_workshop/github_models/model_autokeras.h5')
    MODELS=['model_autokeras.h5']
    ...
    for model_index in range(len(MODELS)):
        ...
        for run_iteration in range(MODEL_ITERATIONS):
            ...
            with strategy.scope():
                # Load the model
                '''model = load_model(f"{MODELS[model_index]}", custom_objects={'tfa.metrics.F1Score':tfa.metrics.F1Score,'ak.CUSTOM_OBJECTS':ak.CUSTOM_OBJECTS})
                model = model.save('/home/u956278/research_workshop/github_models/model_autokeras.h5')'''
                model = load_model('/home/u956278/research_workshop/github_models/model_autokeras.h5')
                ...
                history = compile_and_train_model(model, train_dataset, val_dataset, BATCH_SIZE, EPOCHS, class_weights, checkpoint)
